...
#latest versio of picam_TPU.py using the imutils library by Adrian Rosebrock
#hacked with code from http://www.pyimagesearch.com
#do pip3 install imutils first

#Les Wright 19 July 2019
...
# loop over the frames using the threaded stream
while True:
	if queuepulls ==1:
		timer2 = time.time()
	...
	cv2.rectangle(frame, (0,frameHeight-25),(frameWidth,frameHeight), (0,0,0), -1)

	
	cv2.putText(frame,'Threshold: '+str(round(confThreshold,1))+'  VID FPS: '+str(round(vfps,2))+'  TPU FPS: '+str(qfps), (10, 10),\
	font, 0.3,(0, 255, 255), 1, cv2.LINE_AA)
	
	cv2.putText(frame,'Positive detections: '+str(detections), (10, frameHeight-10),\
 	cv2.FONT_HERSHEY_SIMPLEX, 0.3,(0, 255, 255), 1, cv2.LINE_AA)

	cv2.putText(frame,'Elapsed time: '+str(round(t2secs)), (150, frameHeight-10),\
 	cv2.FONT_HERSHEY_SIMPLEX, 0.3,(0, 255, 255), 1, cv2.LINE_AA)
	...
	if frames >= 1:
		end1 = time.time()
		t1secs = end1-timer1
		vfps = round(frames/t1secs,1)
	if queuepulls > 1:
		end2 = time.time()
		t2secs = end2-timer2
		qfps = round(queuepulls/t2secs,1)
